...
const HeroCarousel: React.FC = ({}) => {
  ...
  return (
    <div>
      ...
      >
        <div className='absolute z-[1] bg-black bg-opacity-60 w-full h-full flex items-center'>
          ...
          {heroCarouselImages.map(({ src, alt }, index) => (
            <CarouselItem key={index} className='p-0'>
              <Card className='border-none'>
                <CardContent className='relative flex items-center min-[3350px]:justify-center min-h-[600px] min-[3350px]:min-h-[1500px] md:max-h-[910px] p-0'>
                  <Image
                    className='min-h-[600px] min-[3350px]:min-h-[1400px] object-cover object-center'
                    src={src}
                    alt={alt}
                    priority
                  />
